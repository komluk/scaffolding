...
`claude-scaffolding` uzywa placeholderow `__CLAUDE_SCAFFOLDING_*__` w plikach szablonowych,
ktore `install.sh` podmienia na wartosci z `~/.claude-scaffolding.env`. To pozwala
...
|-------------|-----------|---------|-------------|----------------------|
| `CLAUDE_SCAFFOLDING_TEST_BACKEND_CMD` | Komenda uruchamiajaca testy backendu | `echo '[claude-scaffolding] no backend tests configured' && true` | Szuka `venv/`, `.venv/`, `app/backend/venv/`, `backend/venv/` -> `source <venv>/bin/activate && pytest` | `agents/developer.md`, `agents/reviewer.md`, `skills/spec-develop/SKILL.md`, `CLAUDE.md` |
| `CLAUDE_SCAFFOLDING_TEST_FRONTEND_CMD` | Komenda walidacji frontendu | `echo '[claude-scaffolding] no frontend validation configured' && true` | Jesli `package.json` ma `"validate"` -> `npm run validate`; jesli `tsconfig.json` -> `npx tsc --noEmit` | `agents/developer.md`, `skills/spec-develop/SKILL.md`, `CLAUDE.md` |
...
```bash
cd ~/.claude  # lub gdzie jest claude-scaffolding
./install.sh  # zapyta o kazda wartosc, domyslne pokaze aktualna
```

### 2. Edytowac `~/.claude-scaffolding.env` recznie

```bash
nano ~/.claude-scaffolding.env
# zmien wartosc, np.:
...
1. Dodaj `CLAUDE_SCAFFOLDING_MY_KEY=default` do `.claude-scaffolding.env.example`.
