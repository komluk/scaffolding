#!/usr/bin/env bash
# uninstall.sh -- remove scaffolding-rendered files from target directory
# Does NOT touch agent-memory/, projects/, sessions/ or ~/.scaffolding.env.
set -euo pipefail

TARGET="${1:-$HOME/.claude}"

echo "[warn] This will remove scaffolding-rendered files from $TARGET"
...
echo "[ok] uninstall done. ~/.scaffolding.env preserved -- remove manually if desired."
